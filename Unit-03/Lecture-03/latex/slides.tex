...
\begin{enumerate}[<+->]
  ...
  \item Study the main build in this order: \textit{Concurrency vs parallelism (quick intuition)} $\rightarrow$ \textit{Threads in Java: \texttt{Thread} and \texttt{Runnable}} $\rightarrow$ \textit{Thread lifecycle (simplified)} $\rightarrow$ \textit{Thread priorities (what they mean and what they do not)}.
...
Where would \textit{Multithreading + Synchronization} matter most in Campus Resource Manager, and which design choice would you justify using this lecture's ideas?
\end{block}
\begin{block}{Quick self-check}
Which part needs one more example before you move on: \textit{Concurrency vs parallelism (quick intuition)}, \textit{Threads in Java: \texttt{Thread} and \texttt{Runnable}}, the worked example, or the recap?
\end{block}
  \slidesources{\OOPSUnitIIISources}
\end{frame}
