...
\begin{enumerate}[<+->]
  ...
  \item Study the main build in this order: \textit{Stream model (big picture)} $\rightarrow$ \textit{Byte streams vs character streams} $\rightarrow$ \textit{Text file I/O with \texttt{FileReader}/\texttt{FileWriter}} $\rightarrow$ \textit{Reading patterns (three common styles)}.
...
Where would \textit{Java I/O Streams + FileReader/FileWriter} matter most in Campus Resource Manager, and which design choice would you justify using this lecture's ideas?
\end{block}
\begin{block}{Quick self-check}
Which part needs one more example before you move on: \textit{Stream model (big picture)}, \textit{Byte streams vs character streams}, the worked example, or the recap?
\end{block}
  \slidesources{\OOPSUnitIIISources}
\end{frame}
...
\begin{frame}[fragile,shrink=8]{Example (1/2): read + compute (safe I/O)}
\begin{lstlisting}
import java.io.*;
...
\begin{frame}[fragile,shrink=8]{Example (2/2): write report + handle errors}
\begin{lstlisting}
      double avg = (count == 0) ? 0.0 : (sum / (double) count);
      bw.write("Count=" + count); bw.newLine();
      bw.write("Max=" + (count == 0 ? "N/A" : max)); bw.newLine();
