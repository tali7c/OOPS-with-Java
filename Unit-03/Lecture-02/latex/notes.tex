...
\textbf{Unit 03:} exception handling and exception handlers
...
\begin{itemize}
  \item Distinguish exceptions vs errors; checked vs unchecked.
  \item Write \texttt{try/catch/finally} with multiple catch blocks.
  \item Explain \texttt{throw} vs \texttt{throws} and apply them correctly.
  \item Create a basic custom exception and use it for validation.
\end{itemize}
...
\midrule
Checked (\texttt{Exception}) & Caller is expected to handle it; condition is reasonably recoverable (e.g., \texttt{IOException}). \\
Unchecked (\texttt{RuntimeException}) & Programming mistake or invalid argument; often not recoverable at runtime (e.g., \texttt{NullPointerException}). \\
\bottomrule
\end{tabular}
...
\end{itemize}
\textbf{When to use this pattern:} for invalid arguments and broken assumptions (\texttt{IllegalArgumentException} is common).
