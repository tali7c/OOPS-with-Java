...
\begin{enumerate}[<+->]
  ...
  \item Study the main build in this order: \textit{Access control (what each means)} $\rightarrow$ \textit{Packages: why and how} $\rightarrow$ \textit{Interface intro: contract view}.
...
Where would \textit{Access Modifiers + Packages + Interface Intro} matter most in Campus Resource Manager, and which design choice would you justify using this lecture's ideas?
\end{block}
\begin{block}{Quick self-check}
Which part needs one more example before you move on: \textit{Access control (what each means)}, \textit{Packages: why and how}, the worked example, or the recap?
\end{block}
  \slidesources{\OOPSUnitIISources}
\end{frame}
